...
async def tutorial_system(call: CallbackQuery):
    ...
    lessons = db_query(
        "SELECT id, title, url FROM youtube_lessons "
        "WHERE is_active=1 ORDER BY sort_order ASC, id ASC LIMIT 50",
        fetchall=True,
    ) or []
    text = (
        f"{get_emoji('tutorial')} <b><u>— YOUTUBE COURSE —</u></b> {get_emoji('tutorial')}\n\n"
        "Learn the complete setup step by step. Open each lesson in order:\n\n"
        "1️⃣ Add funds via <b>Add Balance</b>\n"
        "2️⃣ Open <b>Product Store</b>\n"
        "3️⃣ Choose your panel and validity\n"
        "4️⃣ Receive your key and installation file"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[])
    for index, lesson in enumerate(lessons, start=1):
        kb.inline_keyboard.append([
            InlineKeyboardButton(
                text=f"▶️ Lesson {index}: {str(lesson[1])[:40]}",
                url=lesson[2],
...
async def admin_youtube_course(call: CallbackQuery):
    ...
    text = "🎓 <b>YOUTUBE COURSE MANAGER</b>\n\n"
    if lessons:
        text += "\n".join(
            f"{index}. <b>{escape(str(row[1]))}</b>\n   <code>{escape(str(row[2]))}</code>"
...
async def save_youtube_lesson_title(m: Message, state: FSMContext):
    ...
    await m.answer(
        "🔗 Now send the YouTube video or playlist URL.\n"
...
async def save_youtube_lesson_url(m: Message, state: FSMContext):
    ...
    if not _valid_youtube_url(url):
        return await m.answer("❌ Valid YouTube URL bhejo, jaise https://youtu.be/... ya https://www.youtube.com/...")
    data = await state.get_data()
    ...
    db_query(
        "INSERT INTO youtube_lessons (title, url, sort_order, is_active, created_at) VALUES (?, ?, ?, 1, ?)",
        (data["youtube_lesson_title"], url, next_order, datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
    )
    await state.clear()
    await m.answer(
