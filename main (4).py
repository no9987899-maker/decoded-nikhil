...
def main_menu_kb(user_id: Optional[int] = None) -> InlineKeyboardMarkup:
    ...
    bot_api_label = "🤖 Bot API System" if is_reseller else "🤖 Bot API System 🔒 Reseller Only"
    kb.inline_keyboard.append([
        InlineKeyboardButton(
            text=bot_api_label, callback_data="menu_bot_api_system",
            icon_custom_emoji_id=get_emoji_icon("telegram"), style="primary"
        )
    ])
    extras_row = []
...
async def quick_add_product_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        ...
        "एक ही message में यह format भेजें:\n"
...
async def quick_add_product(m: Message, state: FSMContext):
    if not is_admin(m.from_user.id):
        return
    parts = [part.strip() for part in (m.text or "").split("|")]
    if len(parts) not in (9, 10):
        return await m.answer(
            "❌ Format गलत है। Exactly 9 fields भेजें:\n"
    ...
    # Permit an optional APK link before the API PID for admins who need one.
    if len(parts) == 9:
    ...
    if not matched_category:
        return await m.answer(
            "❌ Category गलत है। इनमें से एक use करें:\n" +
            "\n".join(f"• {cat}" for cat in FIXED_CATEGORIES)
        )
    if not all((panel_name, name, validity, pid, api_duration)):
        return await m.answer("❌ Panel, name, validity, API PID और API duration खाली नहीं हो सकते।")
    try:
        price = float(user_price)
        reseller_price_value = float(reseller_price)
        selected_slot = int(api_slot or "0")
    except ValueError:
        return await m.answer("❌ User price, reseller price और API slot numeric होने चाहिए।")
    if price < 0 or reseller_price_value < 0:
        return await m.answer("❌ Price negative नहीं हो सकती।")
    if selected_slot < 0 or selected_slot > API_SLOT_MAX:
        return await m.answer(f"❌ API slot 0 से {API_SLOT_MAX} के बीच होना चाहिए।")
    if selected_slot:
        configured = {
            int(row[0]): row for row in get_api_slots()
            if row[7] and row[2]
        }
        if selected_slot not in configured:
            return await m.answer(f"❌ API Slot {selected_slot} configured नहीं है। पहले Multi API Manager में setup करें।")

    normalized_duration = normalize_api_duration(api_duration)
    conn = sqlite3.connect("yp_shop.db")
    cursor = conn.cursor()
    cursor.execute(
        ...
        (
            ...
            "1 Device HWID", 1, pid, 0, normalized_duration, selected_slot
        )
    )
    ...
    await state.clear()
    await m.answer(
        f"✅ <b>Quick Product Created!</b>\n\n"
        f"📦 Product ID: <code>{product_id}</code>\n"
        f"🏷️ {escape(matched_category)} → {escape(panel_name)} → {escape(name)}\n"
        f"💰 User: ₹{price:.2f} | Reseller: ₹{reseller_price_value:.2f}\n"
        f"🔌 API Route: <code>{'Auto / Failover All APIs' if selected_slot == 0 else f'Slot {selected_slot}'}</code>",
