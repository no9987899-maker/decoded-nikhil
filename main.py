...
def init_db() -> None:
    ...
    migrations = [
        ...
        "ALTER TABLE products ADD COLUMN maintenance_mode INTEGER NOT NULL DEFAULT 0",
...
async def send_main_menu(ctx: Any):
    ...
    product_count = db_query(
        "SELECT COUNT(*) FROM products WHERE is_active=1 AND COALESCE(maintenance_mode, 0)=0",
        fetchone=True,
    )
...
async def view_shop_panels(call: CallbackQuery):
    ...
    for cat in FIXED_CATEGORIES:
        count = db_query("SELECT COUNT(*) FROM products WHERE category LIKE ? AND is_active=1 AND COALESCE(maintenance_mode,0)=0", (cat + '%',), fetchone=True)[0]
...
async def view_panel_names(call: CallbackQuery):
    ...
    # Never expose a panel that only contains offline products to customers.
    # Maintenance products remain visible by design, but cannot be purchased.
    ...
    if not panel_names:
        prods = db_query("SELECT id, name, price_inr, stock, reseller_price, validity, device_limit, external_enabled, COALESCE(maintenance_mode,0) FROM products WHERE category LIKE ? AND is_active=1", (category + '%',), fetchall=True)
...
async def view_products_for_panel(call: CallbackQuery):
    ...
    prods = db_query("SELECT id, name, price_inr, stock, reseller_price, validity, device_limit, external_enabled, COALESCE(maintenance_mode,0) FROM products WHERE category LIKE ? AND panel_name LIKE ? AND is_active=1", (category + '%', panel_name + '%'), fetchall=True)
...
async def show_products_for_panel(call: CallbackQuery, prods: List[Tuple], header: str):
    ...
    for p in prods:
        prod_id, package_name, normal_price, stock, reseller_price, validity, device, external_enabled, maintenance_mode = p
        ...
        stock_status = (
            "🛠️ Under Maintenance" if maintenance_mode
            else ("♾️ API Available" if external_enabled else ("✅ In Stock" if stock > 0 else "❌ Out of Stock"))
        )
        ...
        if maintenance_mode:
            kb.inline_keyboard.append([InlineKeyboardButton(
                text=f"🛠️ {package_name} (Maintenance)",
                callback_data=f"product_maintenance_{prod_id}",
                style="danger",
            )])
...
@dp.callback_query(F.data.startswith("product_maintenance_"))
async def product_maintenance_notice(call: CallbackQuery):
    await call.answer("🛠️ This product is under maintenance right now. Please try again later.", show_alert=True)
...
async def show_purchase_options(call: CallbackQuery):
    ...
    prod = db_query("""
        SELECT name, price_inr, stock, validity, device_limit, reseller_price,
               external_enabled, category, panel_name, COALESCE(maintenance_mode, 0)
    ...
    if prod[9]:
        return await call.answer("🛠️ This product is under maintenance. Please try again later.", show_alert=True)
...
async def direct_upi_purchase(call: CallbackQuery):
    ...
    prod = db_query(
        "SELECT price_inr, reseller_price, COALESCE(maintenance_mode,0) FROM products WHERE id=? AND is_active=1",
        (prod_id,),
        fetchone=True,
    )
    user = db_query(
...
async def process_buy(call: CallbackQuery):
    ...
    prod = db_query("""
        SELECT name, price_inr, stock, apk_link, validity, device_limit,
               ...
               is_active, COALESCE(maintenance_mode, 0)
    ...
    if not prod[14]:
        return await call.answer("❌ This product is offline.", show_alert=True)
    if prod[15]:
        return await call.answer("🛠️ This product is under maintenance. Please try again later.", show_alert=True)
...
async def _show_category_products(call: CallbackQuery, category: str, page: int = 0):
    if not is_admin(call.from_user.id): return
    rows=db_query(
        "SELECT id,panel_name,name,price_inr,stock,is_active,COALESCE(maintenance_mode,0) "
    ...
    category_key = "root" if category == "ANDROID ROOT PANEL" else "nonroot"
    page_rows = rows[page * page_size:(page + 1) * page_size]

    kb=InlineKeyboardMarkup(inline_keyboard=[])
    for pid,panel,name,price,stock,active,maintenance in page_rows:
        label=f"#{pid} {panel or name} | ₹{float(price):.0f} | Stock {stock}"
        if maintenance:
            status_label = "🛠️ Maint."
            status_callback = f"admin_product_status_menu_{pid}"
        elif active:
            status_label = "🟢 Online"
            status_callback = f"admin_cat_status_{pid}_offline_{category_key}_{page}"
        else:
            status_label = "⚪ Offline"
            status_callback = f"admin_cat_status_{pid}_online_{category_key}_{page}"
        kb.inline_keyboard.append([
            InlineKeyboardButton(text=label[:64],callback_data=f"admin_catprod_{pid}",style='primary'),
            InlineKeyboardButton(text=status_label,callback_data=status_callback,style='success' if active and not maintenance else 'danger'),
        ])
    navigation = []
    if page > 0:
        navigation.append(InlineKeyboardButton(
            text="⬅️ Previous",
            callback_data=f"admin_manage_{category_key}_products_page_{page - 1}",
            style='primary',
        ))
    ...
    await call.message.edit_text(
        f"📦 <b>{escape(category)}</b>\n\nProduct चुनें या उसी row के नीचे Online/Offline button दबाएँ.\n"
        f"Page {page + 1}/{total_pages} • {len(rows)} products",
        reply_markup=kb,
        parse_mode='HTML',
    )
...
@dp.callback_query(F.data.startswith("admin_manage_root_products_page_"))
async def admin_manage_root_products_page(call: CallbackQuery):
    try:
        page = int(call.data.rsplit("_", 1)[1])
    except (TypeError, ValueError):
        return await call.answer("Invalid page.", show_alert=True)
    await _show_category_products(call, "ANDROID ROOT PANEL", page)

@dp.callback_query(F.data.startswith("admin_manage_nonroot_products_page_"))
async def admin_manage_nonroot_products_page(call: CallbackQuery):
    try:
        page = int(call.data.rsplit("_", 1)[1])
    except (TypeError, ValueError):
        return await call.answer("Invalid page.", show_alert=True)
    await _show_category_products(call, "ANDROID NON ROOT PANEL", page)

@dp.callback_query(F.data.regexp(r"^admin_cat_status_[0-9]+_(online|offline)_(root|nonroot)_[0-9]+$"))
async def admin_category_product_status(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return await call.answer("Admin only.", show_alert=True)
    try:
        _, _, _, product_id_text, new_status, category_key, page_text = call.data.split("_")
        product_id = int(product_id_text)
        page = int(page_text)
    except (TypeError, ValueError):
        return await call.answer("Invalid product status.", show_alert=True)

    category = "ANDROID ROOT PANEL" if category_key == "root" else "ANDROID NON ROOT PANEL"
    product = db_query(
        "SELECT category FROM products WHERE id=?",
        (product_id,),
        fetchone=True,
    )
    if not product or product[0] != category:
        return await call.answer("Product not found in this category.", show_alert=True)

    is_active = 1 if new_status == "online" else 0
    db_query(
        "UPDATE products SET is_active=?, maintenance_mode=0 WHERE id=?",
        (is_active, product_id),
    )
    await call.answer(f"Product is now {new_status.title()}.")
    await _show_category_products(call, category, page)
...
async def admin_category_product_view(call: CallbackQuery):
    ...
    row=db_query("SELECT category,panel_name,name,price_inr,stock,is_active,COALESCE(maintenance_mode,0) FROM products WHERE id=?",(pid,),fetchone=True)
    if not row: return await call.answer('Product not found',show_alert=True)
    cat,panel,name,price,stock,active,maintenance=row
    status_text = "Maintenance" if maintenance else ("Online" if active else "Offline")
    kb=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"⚙️ Change Status: {status_text}",callback_data=f"admin_product_status_menu_{pid}",style='primary')],
...
async def _render_admin_products(call: CallbackQuery, page: int = 0):
    if not is_admin(call.from_user.id):
        return await call.answer("❌ Yeh button sirf admin ke liye hai.", show_alert=True)

    await call.answer()
    try:
        prods = db_query(
            "SELECT id, name, category, panel_name, stock, is_active, "
            "COALESCE(maintenance_mode, 0) FROM products",
            fetchall=True,
        )
        if prods is None:
            return await call.message.answer(
                "❌ Products database read nahi ho paaya. Bot logs check karein."
            )
        ...
        # Keep the inline keyboard small enough for Telegram. This store can
        # contain 100+ products, which exceeds Telegram's reply-markup limit
        # when every product is placed on a single keyboard.
        page_size = 10
        total_pages = (len(prods) + page_size - 1) // page_size
        page = max(0, min(page, total_pages - 1))
        start_index = page * page_size
        page_products = prods[start_index:start_index + page_size]

        kb = InlineKeyboardMarkup(inline_keyboard=[])
        for p in page_products:
            status_dot = "🛠️" if p[6] else ("🟢" if p[5] else "⚪")
            category = str(p[2] or "")
            panel_name = str(p[3] or "")
            product_name = str(p[1] or "")
            label = f"{status_dot} #{p[0]} [{category}] {panel_name or product_name} | Stock: {p[4]}"
            kb.inline_keyboard.append([
                InlineKeyboardButton(
                    text=label[:64],
                    callback_data=f"admin_view_p_{p[0]}",
                    style="primary",
                )
            ])
        ...
        first_product = start_index + 1
        last_product = start_index + len(page_products)
        await call.message.edit_text(
            "📦 <b>Database Editor: Select Node to modify</b>\n\n"
            f"Showing {first_product}–{last_product} of {len(prods)} "
            f"(Page {page + 1}/{total_pages})",
            reply_markup=kb,
            parse_mode="HTML",
        )
    except Exception:
        logger.exception("Failed to open Manage Products")
        await call.message.answer(
            "❌ Manage Products kholte waqt error aaya. Bot logs check karein."
        )

@dp.callback_query(F.data == "admin_manage_prods")
async def admin_manage_prods(call: CallbackQuery):
    await _render_admin_products(call, page=0)
...
async def admin_manage_prods_page(call: CallbackQuery):
    try:
        ...
        return await call.answer("❌ Invalid page.", show_alert=True)
    await _render_admin_products(call, page=page)
...
async def admin_view_product(call: CallbackQuery):
    ...
    try:
        ...
        status_text = "🛠️ Maintenance" if prod[16] else ("🟢 Online" if prod[10] else "⚪ Offline")
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text=f"⚙️ Product Status: {status_text}", callback_data=f"admin_product_status_menu_{p_id}", style="primary")],
...
@dp.callback_query(F.data.startswith("admin_product_status_menu_"))
async def admin_product_status_menu(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return await call.answer("❌ Admin only.", show_alert=True)
    try:
    ...
    row = db_query(
        "SELECT is_active, COALESCE(maintenance_mode, 0) FROM products WHERE id=?",
        (p_id,),
        fetchone=True,
    )
    if not row:
    ...
    current_status = "Maintenance" if row[1] else ("Online" if row[0] else "Offline")
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🟢 Online — show and sell", callback_data=f"admin_product_status_{p_id}_online", style="success")],
        [InlineKeyboardButton(text="⚪ Offline — hide from store", callback_data=f"admin_product_status_{p_id}_offline", style="danger")],
        [InlineKeyboardButton(text="🛠️ Maintenance — visible, not for sale", callback_data=f"admin_product_status_{p_id}_maintenance", style="primary")],
        [InlineKeyboardButton(text="🔙 Back to Product", callback_data=f"admin_view_p_{p_id}", style="danger")],
    ])
    await call.answer()
    await call.message.edit_text(
        f"⚙️ <b>Product #{p_id} status</b>\n\nCurrent: <b>{current_status}</b>\n\n"
        "Online: customers can view and buy.\n"
        "Offline: hidden from the store.\n"
        "Maintenance: visible, but purchase is blocked.",
        reply_markup=kb,
        parse_mode="HTML",
    )

@dp.callback_query(F.data.regexp(r"^admin_product_status_[0-9]+_(online|offline|maintenance)$"))
async def admin_set_product_status(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return await call.answer("❌ Admin only.", show_alert=True)
    try:
        _, _, _, p_id_text, new_status = call.data.split("_", 4)
        p_id = int(p_id_text)
    except (TypeError, ValueError):
        return await call.answer("Invalid product status.", show_alert=True)
    ...
    status_values = {
        "online": (1, 0, "🟢 Online"),
        "offline": (0, 0, "⚪ Offline"),
        "maintenance": (1, 1, "🛠️ Maintenance"),
    }
    if new_status not in status_values:
        return await call.answer("Invalid product status.", show_alert=True)
    is_active, maintenance_mode, label = status_values[new_status]

    exists = db_query("SELECT id FROM products WHERE id=?", (p_id,), fetchone=True)
    if not exists:
        return await call.answer("Product not found.", show_alert=True)
    db_query(
        "UPDATE products SET is_active=?, maintenance_mode=? WHERE id=?",
        (is_active, maintenance_mode, p_id),
    )
    await call.answer(f"Status set to {label}.", show_alert=True)
...
async def admin_toggle_product(call: CallbackQuery):
    ...
    db_query("UPDATE products SET is_active=?, maintenance_mode=0 WHERE id=?", (new_val, p_id))
...
async def reseller_api_products(request: web.Request):
    ...
    rows=db_query("""SELECT id,category,panel_name,name,price_inr,reseller_price,stock,validity,device_limit,external_enabled,external_product_id,requires_android_id
                     FROM products
                     WHERE is_active=1 AND COALESCE(maintenance_mode,0)=0
                     ORDER BY category,panel_name,id""", fetchall=True) or []
    items=[]
    for r in rows:
...
async def _api_purchase(user_id: int, product_id: int, android_id: str = "") -> dict:
    prod=db_query("""SELECT name,price_inr,stock,apk_link,validity,device_limit,category,reseller_price,panel_name,external_enabled,
                           external_product_id,requires_android_id,external_duration,COALESCE(maintenance_mode,0)
                     FROM products WHERE id=? AND is_active=1""", (product_id,), fetchone=True)
    ...
    if prod[13]: return {"ok":False,"status":503,"error":"Product is temporarily under maintenance."}
...
async def _connected_bot_products(user_id: int):
    rows=db_query("SELECT id,name,reseller_price,price_inr,requires_android_id FROM products WHERE is_active=1 AND COALESCE(maintenance_mode,0)=0 ORDER BY id LIMIT 30",fetchall=True) or []
